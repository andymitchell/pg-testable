export type Queue = <T>(queueName: string, onRun: (...args: any[]) => T | PromiseLike<T>, descriptor?: string) => Promise<T>;
...
function completeItem(q:QueueItem, output: any, error?:any) {
    if( !queues[q.queueName] || queues[q.queueName][0]!==q ) throw new Error("Something went very wrong in queue");

    error? q.reject(error) : q.resolve(output);
    queues[q.queueName] = queues[q.queueName].filter(x => x!==q);
    next();
}
